...
class CustomSDControlNetPipeline(StableDiffusionControlNetPipeline):
    def __call__(self, 
                 prompt_embeds=None,
                 image=None,
        ...
        return super().__call__(
            prompt=None,  # disable text prompt
...
def log_validation(
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        # Tokenize and embed using chunking
        tokenizer.model_max_length = 154
        tokens = tokenizer(validation_prompt, return_tensors="pt", truncation=True)["input_ids"][0]
        chunks = [tokens[i:i+77] for i in range(0, len(tokens), 77)]
        
        chunk_embeds = []
        for chunk in chunks:
            padded = torch.nn.functional.pad(chunk, (0, 77 - chunk.size(0)), value=tokenizer.pad_token_id).unsqueeze(0).to(accelerator.device)
            with torch.no_grad():
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                for image in images:
                    formatted_images.append(np.asarray(image.resize((192,192), Image.BILINEAR)))
...
def main(args):
    ...
    def process_long_prompt_batch(text_encoder, input_ids, weight_dtype_, max_length=77, chunk_strategy="average"):
        """
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):    
            with accelerator.accumulate(controlnet):
                ...
                encoder_hidden_states = process_long_prompt_batch(text_encoder, batch["input_ids"], weight_dtype_ = weight_dtype)
